...
**`ui.js`** — `UIController`. Core DOM infrastructure: `cacheElements()`, `renderAll()`, `setActivePanel()`, shared helpers, and event wiring. Panel render logic lives in separate modules (see below) mixed in at the bottom via `Object.assign(UIController.prototype, InboxPanel, NextPanel, ...)`. `PANEL_RENDER_FNS` is a frozen map of panel-id → render method name — **adding a new panel requires: (1) a `panels/<name>.js` module exporting a plain object of methods, (2) an import + entry in `Object.assign` at the bottom of `ui.js`, and (3) an entry in `PANEL_RENDER_FNS`**. Use the `/add-panel` slash command for the full checklist.

`_dirtyPanels` is a Set tracking which panels need re-rendering. On `statechange`, `renderAll()` marks all panels dirty and renders only the active one; hidden panels render on-demand when `setActivePanel()` is called. Always-unconditional calls in `renderAll()`: `renderSummary()`, `renderAssociationFlyout()`, `updateSuggestionLists()`, `updateCounts()`, `syncTheme()`, `applyPanelVisibility()`.
...
**`panels/*.js`** — One module per panel (e.g. `panels/inbox.js`). Each exports a plain object of methods (`renderX`, `initXListeners`, etc.) that are mixed into `UIController.prototype`. Methods access `this.taskManager`, `this.elements`, and shared helpers as if they were defined on `UIController` directly. Event listeners use container-level delegation (attach once to a stable parent, check `event.target.closest(selector)`) rather than per-item listeners.

**`analytics.js`** — `AnalyticsController`. Chart rendering only, wrapping `chart.min.js`. Reads from `TaskManager`; no state mutations.

**`review.js`** — `ReviewController`. Drives the full-screen Weekly Review mode. Walks five sections in order: Inbox (gated — must reach zero) → Pending Tasks → Delegated → Backburner → Projects. Session state (current position, processed IDs, stats) is stored in localStorage under `nextflow-review-session` with a 12-hour TTL so reviews can be paused/resumed. Streak data (`settings.review`) is synced across devices via `taskManager.updateReviewData()` on completion.
